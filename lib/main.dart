...
void main() async {
  WidgetsFlutterBinding
      .ensureInitialized(); // Обязательно для асинхронных операций перед runApp
  SharedPreferences prefs = await SharedPreferences.getInstance();
  String? token = prefs.getString('token');
  email_user = prefs.getString('email_user');

  runApp(
      MyApp(startPage: token == null ? LoginPage() : BottomNavigationPage()));
}
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        // hintColor: Colors.amber, // Акцентный цвет
